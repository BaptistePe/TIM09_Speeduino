...
//List of logger field names matching TunerStudio MSL [Datalog] format
constexpr char header_0[] PROGMEM = "SecL";
constexpr char header_1[] PROGMEM = "RPM";
constexpr char header_2[] PROGMEM = "TPS";
constexpr char header_3[] PROGMEM = "VE (Current)";
constexpr char header_4[] PROGMEM = "PW";
constexpr char header_5[] PROGMEM = "Duty Cycle";
constexpr char header_6[] PROGMEM = "Advance (Current)";
constexpr char header_7[] PROGMEM = "Battery V";
constexpr char header_8[] PROGMEM = "Aux0";
constexpr char header_9[] PROGMEM = "Aux1";
constexpr char header_10[] PROGMEM = "Aux2";
constexpr char header_11[] PROGMEM = "Advance 1";
constexpr char header_12[] PROGMEM = "Sync status";
constexpr char header_13[] PROGMEM = "CumInjTime1";
constexpr char header_14[] PROGMEM = "CumInjTime2";
constexpr char header_15[] PROGMEM = "nbInjection";
...
void writeSDLogEntry()
{
  ...
  if(SD_status == SD_STATUS_ACTIVE)
  {
    ...
    if(rb.bytesFree() > SD_LOG_ENTRY_TOTAL_BYTES)
    {
      ...
      //Write the log fields matching TunerStudio MSL [Datalog] format
      // SecL
      rb.print(currentStatus.secl); rb.print(',');
      // RPM
      rb.print(currentStatus.RPM); rb.print(',');
      // TPS: stored as 0-200 (0%-100%)
      #if FPU_MAX_SIZE >= 32
        rb.print(currentStatus.TPS / 2.0f, 1);
      #else
        rb.print(currentStatus.TPS / 2U);
      #endif
      rb.print(',');
      // VE (Current)
      rb.print(currentStatus.VE); rb.print(',');
      // PW: convert from us to ms
      #if FPU_MAX_SIZE >= 32
        rb.print(currentStatus.PW1 / 1000.0f, 3);
      #else
        rb.print(currentStatus.PW1 / 1000U);
      #endif
      rb.print(',');
      // Duty Cycle
      #if FPU_MAX_SIZE >= 32
        if (currentStatus.RPM > 0U) {
          uint32_t revolutionTime = 60000000UL / currentStatus.RPM;
          float dc = (currentStatus.PW1 * 100.0f * currentStatus.nSquirts) / revolutionTime;
          if (configPage2.strokes == FOUR_STROKE) { dc /= 2.0f; }
          rb.print(dc, 1);
        } else { rb.print("0.0"); }
      #else
        {
          ...
          if (currentStatus.RPM > 0U) {
            uint32_t revolutionTime = 60000000UL / currentStatus.RPM;
            dc = (uint16_t)((currentStatus.PW1 * 100UL * currentStatus.nSquirts) / revolutionTime);
            if (configPage2.strokes == FOUR_STROKE) { dc /= 2U; }
          }
          rb.print(dc);
        }
      #endif
      rb.print(',');
      // Advance (Current)
      rb.print(currentStatus.advance); rb.print(',');
      // Battery V: battery10 is in 0.1V units
      #if FPU_MAX_SIZE >= 32
        rb.print(currentStatus.battery10 / 10.0f, 1);
      #else
        rb.print(currentStatus.battery10 / 10U);
      #endif
      rb.print(',');
      // Aux0, Aux1, Aux2 (CAN inputs 0-2)
      rb.print(currentStatus.canin[0]); rb.print(',');
      rb.print(currentStatus.canin[1]); rb.print(',');
      rb.print(currentStatus.canin[2]); rb.print(',');
      // Advance 1
      rb.print(currentStatus.advance1); rb.print(',');
      // Sync status: halfSync(bit4 of status3) + (hasSync * 2)
      rb.print((uint8_t)((BIT_CHECK(currentStatus.status3, BIT_STATUS3_HALFSYNC) ? 1U : 0U) + (currentStatus.hasSync ? 2U : 0U))); rb.print(',');
      // CumInjTime1: lower 16 bits of cumulativePW (us, wraps at 65535)
      rb.print((uint16_t)(currentStatus.cumulativePW & 0xFFFFU)); rb.print(',');
      // CumInjTime2: upper 16 bits of cumulativePW (overflow counter)
      rb.print((uint16_t)((currentStatus.cumulativePW >> 16U) & 0xFFFFU)); rb.print(',');
      // nbInjection
      rb.print(currentStatus.nbInjection);
      rb.println("");
    }
